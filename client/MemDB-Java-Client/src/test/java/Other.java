...
//        node5_read();
    ...
    static void linearized_cas() {
        MemDb memDb = MemDbConnections.sync("192.168.1.159", 10000, "789");
        memDb.set("n", "0");
    ...
    static void concurrent_cas() {
        MemDb globalMemDb = MemDbConnections.sync("192.168.1.159", 10000, "789");
        globalMemDb.set("lock", "false");

        Thread t1 = new Thread(() -> {
            try {
                MemDb local = MemDbConnections.sync("192.168.1.159", 10000, "789");
                for(int i = 0; i < 100; i++){
                    Thread.yield();
        ...
        Thread t2 = new Thread(() -> {
            try {
                MemDb local = MemDbConnections.sync("192.168.1.159", 10001, "789");
                for(int i = 0; i < 100; i++) {
                    Thread.yield();
                    while(!local.cas("lock", "false", "true")) {
                        Thread.sleep(100);
    ...
    static void node5_write() {
        MemDb memDb = MemDbConnections.sync("192.168.1.159", 10004, "789");
        memDb.set("d", "xd");
    }
    ...
    static void node3_write() {
        MemDb memDb = MemDbConnections.sync("192.168.1.159", 10002, "789");
        memDb.set("3", "Mi numero");
    }

    @SneakyThrows
    static void node4_read() {
        MemDb memDb = MemDbConnections.sync("192.168.1.159", 10003, "789");
        System.out.println(memDb.get("cascos"));
    }
