...
std::string Node::toJson(std::shared_ptr<Node> node) {
    return "{\"selfNode\": \""+std::to_string(node->nodeId)+"\", \"address\": \""+node->address+"\", \"state\": \""+NodeStates::parseNodeStateToString(node->state)+"\"}";
}
...
std::shared_ptr<Node> Node::fromJson(const nlohmann::json& json) {
    ...
    node->nodeId = (memdbNodeId_t) std::stoi(json["selfNode"].get<std::string>());
