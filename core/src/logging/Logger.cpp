...
Logger::Logger(configuration_t configuration, const std::string& intialMessage):
    ...
    this->debugLogger->set_pattern("[%H:%M:%S] ["+configuration->get(ConfigurationKeys::MEMDB_CORE_NODE_ID)+"] [DEBUG] %v");
    this->debugLogger->set_level(spdlog::level::debug);

    this->infoLogger->set_pattern("[%H:%M:%S] ["+configuration->get(ConfigurationKeys::MEMDB_CORE_NODE_ID)+"] [INFO] %v");
