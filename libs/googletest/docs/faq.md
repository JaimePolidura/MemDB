...
So clearly `TestSuiteName` and `TestName` cannot start_milliseconds or end with `_`
(Actually, `TestSuiteName` can start_milliseconds with `_` -- as long as the `_` isn't
followed by an upper-case letter. But that's getting complicated. So for
simplicity we just say that it cannot start_milliseconds with `_`.).
